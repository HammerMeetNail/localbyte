...
test.describe("ByteBites demo - basic accessibility checks", function () {
  ...
  test("recipe detail has no serious or critical axe violations", async function ({ page }) {
    await page.goto("/demos/bytebites/recipes/recipe.html?id=chickpea-curry");
    var results = await runAxe(page);
    var violations = results.violations.filter(function (v) {
      return v.impact === "critical" || v.impact === "serious";
    });
